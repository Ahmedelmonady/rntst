...
export class GameballApp {
  ...
  async initializeCustomer(
    request: CustomerInitializationRequest,
    callback?: Callback<CustomerInitializationResponse>
  ): Promise<CustomerInitializationResponse> {
    try {
      this.ensureInitialized();

      const response = await this.makeRequest(API_ENDPOINTS.CUSTOMERS, request);

      // Extract gameballId from response
      const result: CustomerInitializationResponse = {
  ...
  private async makeRequest(endpoint: string, data: any): Promise<any> {
    ...
    const url = `${baseUrl}${API_ENDPOINTS.INTEGRATIONS}${endpoint}`;
  ...
  private async fetchBotSettings(): Promise<void> {
    try {
      const response = await fetch(
        `${API_ENDPOINTS.BOT_SETTINGS}?apiKey=${this.apiKey}`,
        {
          method: 'GET',
        }
      );
      if (!response.ok) {
        throw new Error(
          `${ERROR_MESSAGES.FETCH_BOT_SETTINGS_FAILED} ${response.status}`
        );
      }
      const json = await response.json();

      if (json && json.response) {
        this.mainColor = json.response.botMainColor.replace('#', '');
      }
    } catch (error) {
